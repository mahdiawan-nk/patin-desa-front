...
const { updateSeeding } = useSeedingApi()
...
async function onSubmit() {
    ...
    try {
        loading.value = true
        await updateSeeding(props.seeding.id, form.value)
        emit('updated')
        close()
    } catch (err: any) {
...
watch(
    ...
    async (val) => {
        if (val) {
            const seeding = (val as any).original ?? val // fallback ke original kalau ada
            if (!kolamOptions.value.length) {
                await fetchKolamOptions()
            }

            form.value = {
                kolam_budidaya_id: seeding.kolam_budidaya_id,
                jenis_benih: seeding.jenis_benih,
                tanggal_tebar: seeding.tanggal_tebar,
                jumlah_tebar: seeding.jumlah_tebar,
                rata_rata_berat_awal: seeding.rata_rata_berat_awal,
                kepadatan_tebar: seeding.kepadatan_tebar,
                status_seeding: seeding.status_seeding,
                catatan: seeding.catatan
            }
        }
    },
    { immediate: true }
