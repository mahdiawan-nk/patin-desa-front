...
useDynamicSeo({
    title: 'Panel - Sampling Growth',
    description: 'Budidaya ikan patin segar langsung dari XIII Koto Kampar. Beli ikan patin segar berkualitas tinggi.',
    image: '/og-image.jpg'
})

import { samplingData } from '@/data/sampling'
const perPage = ref(10)
const page = ref(1)
const columns = [
    { accessorKey: 'tanggal_sampling', header: 'Tanggal Sampling' },
    { accessorKey: 'jumlah_sample', header: 'Jumlah Sample' },
    { accessorKey: 'rata_rata_berat', header: 'Rata-rata Berat (g)' },
    { accessorKey: 'rata_rata_panjang', header: 'Rata-rata Panjang (cm)' },
    { accessorKey: 'tingkat_kematian', header: 'Tingkat Kematian' },
...
<template>
    <div class="p-6">
        ...
        <UCard>
            ...
            <UTable :data="samplingData" :columns="columns" class="w-full">
                <!-- Custom cell untuk kolam -->
                <template #kolam-data="{ row }">
                    <div>
                        <!-- <p class="font-semibold">{{ row.kolam.nama_kolam }}</p> -->
                        <!-- <p class="text-sm text-gray-500">Benih: {{ row.kolam }}</p> -->
                    </div>
                </template>
